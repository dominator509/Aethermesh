...
|---|---|---|---|---|---|
| A1 | Python 3.11+ on host | Reference uses 3.11 syntax / `tomllib` | Build fails | `python3 --version` ≥ 3.11 | Yes (EP-001) |
| A2 | `uv` is the package manager | Fast, lockfile-native | Scripts fail | `command -v uv` | Yes |
...
| A7 | GitHub Actions for CI | OSS conventions | `.github/workflows/` wasted if elsewhere | Repo lives on GitHub | Yes (EP-009) |
...
| A12 | Existing L1–L5 reference bundles in `bundles/` are the starting point | They were demoed end-to-end | Re-implementing wastes time | Each `python -m code` succeeds at HEAD | Yes (EP-000) |
