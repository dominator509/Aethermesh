"""Tests for Apple SEP attestation backend — EP-006 M2."""

import sys

import pytest

from aethermesh.L3_handshake.attestation.backends.apple_sep import (
    BACKEND_CODE,
    build_quote,
    verify_quote,
)
...
@pytest.mark.skipif(sys.platform != "darwin", reason="Apple SEP only available on macOS")
class TestAppleSepReal:
    def test_build_quote_real(self) -> None:
        ...
        assert not quote.hardware_quote.startswith(b"APPLE_SEP_PLACEHOLDER")
